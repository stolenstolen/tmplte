...
Если вы захотите добавить сборку через Vite или другой инструмент — могу добавить `build`/`preview` скрипты в `package.json`.
...
Если нужно, добавлю поддержку сборки (Vite) и обновлю `package.json` скриптами `build`/`preview`.